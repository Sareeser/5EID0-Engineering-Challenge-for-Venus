...
/* Reads the calibration data and sets the device into auto sensing mode */
...
int setMeasurementTimingBudget(vl53x *ptr_s, uint32_t budget_us) {
    ...
    if (enables & VL53L0X_SEQUENCE_ENABLE_FINAL_RANGE) {
        ...
        if (used_budget_us > budget_us) return 0; // "Requested timeout too big."
