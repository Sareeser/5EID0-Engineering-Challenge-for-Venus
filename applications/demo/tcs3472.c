...
int tcs_set_integration(tcs3472 *sensor, uint8_t atime) {
    ...
    if(sensor->enabled) return write_byte(sensor->iic_index, tcs3472_repeat_byte(TCS3472_ATIME_REG), sensor->integration_time);
...
int tcs_set_gain(tcs3472 *sensor, tcs3472_gain gain) {
    sensor->gain = gain;
    if (sensor->enabled == 1) return write_byte(sensor->iic_index, tcs3472_repeat_byte(TCS3472_CONTROL_REG), (TCS3472_CONTROL_GAIN & sensor->gain));
    return TCS3472_SUCCES;
}
